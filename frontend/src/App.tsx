...
function App() {
  ...
  useEffect(() => { if (tab !== "workout") setWorkoutMessage(""); }, [tab]);
  ...
  const correct = async () => { if (!workout || !durationMinutes || activity === "unknown") return; try { const w = await api.correctWorkout(workout.id, activity, Math.round(Number(durationMinutes) * 60)); setWorkout(w); setWorkoutMessage("Workout correction saved."); } catch (e) { setError((e as Error).message); } };
  ...
  return <div className="shell"><header><div><p className="eyebrow">POST-WORKOUT</p><h1>Recovery meal</h1><p className="subtle">A practical next meal based on the workout you actually completed.</p></div><div className="status">{profile ? "Profile ready" : "Set up your profile"}</div></header>
    ...
    {error && <div className="alert error">{error}</div>}{tab === "workout" && workoutMessage && <div className="alert success">{workoutMessage}</div>}{message && tab !== "workout" && <div className="alert success">{message}</div>}
